...
// We don't render WebGL content, so skip it and let chromium.args emit
// `--disable-webgl` instead of its SwiftShader/`--in-process-gpu` flags.
// A separately-added raw `--disable-gpu` flag conflicts with those
// SwiftShader flags and made chrome-headless-shell abort immediately with
// no stderr output ("Failed to launch the browser process: Code: 0").
